...
const ChipInput = ({
    ...
    // setting us state for managing tags array
  ...
  return (
    <div className='flex flex-col space-y-2'>
        <label htmlFor={name} className='text-sm lable-style'>{label}</label>
        <div className='flex w-full flex-wrap gap-y-2'>
            {
                tags.map((tag , index) => (
                    <div className='m-1 flex items-center rounded-full bg-yellow-400 px-2 py-1 text-sm text-richblack-5' key={index}>
                        {tag}
                        <button type='button' className='ml-2 focus:outline-none' onClick={() => handleDeleteTag(index)}>
